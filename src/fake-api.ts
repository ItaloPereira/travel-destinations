...
export const getNearbyDestinations = async (id: number): Promise<Destination[]> => {
  ...
  const currentDestination = destinations.find(destination => destination.id === id);
  ...
  return destinations
    ...
    .sort((a, b) =>
      Math.abs(a.latitude - currentDestination.latitude) - Math.abs(b.latitude - currentDestination.latitude)
    )
    .slice(0, 4);
};